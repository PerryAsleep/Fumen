...
/// <summary>
/// Data structure to enqueue UndoableActions, and undo and redo those actions
/// in the order they were enqueued.
/// 
/// UndoableActions may be asynchronous. If an asynchronous UndoableAction is being
/// done or undone, no other actions may be done or undone until it is complete.
/// Call IsDoingOrUndoing to determine if an asynchronous UndoableAction is being
...
/// </summary>
public sealed class ActionQueue : Notifier<ActionQueue>
{
	...
	/// <summary>
	/// UndoStack of UndoableActions for undo and redo.
	/// </summary>
	private readonly UndoStack<UndoableAction> Actions;
	...
	{
		Actions = new UndoStack<UndoableAction>(size, true);
	}
	...
	/// <summary>
	/// Do the given UndoableAction and add it to the queue of UndoableAction for undo and redo.
	/// </summary>
	/// <param name="editorAction">UndoableAction to do.</param>
	public void Do(UndoableAction editorAction)
	{
	...
	/// <summary>
	/// Enqueue given UndoableAction and add it to the queue of UndoableAction for undo and redo without doing the action.
	/// </summary>
	/// <param name="editorAction">UndoableAction to enqueue.</param>
	public void EnqueueWithoutDoing(UndoableAction editorAction)
	{
	...
	private void EnqueueWithoutDoing(UndoableAction editorAction, bool log)
	{
		Assert(!IsDoingOrUndoing());
	...
	/// </summary>
	/// <param name="lastAction">UndoableAction to set as the new LastAction.</param>
	private void UpdateLastAction(UndoableAction lastAction)
	{
	...
	/// <summary>
	/// Clear the queue of UndoableActions.
	/// </summary>
	public void Clear()
	{
		LastSavedIndex = -1;
